    ...
    <ul className="list-disc list-inside space-y-1 text-lg text-[#686868] pl-5">
      {items.map((item) => (
        <li key={item} className="marker:text-black">
          {item}
        </li> // Uses theme color for bullet points
      ))}
    </ul>
  );
    ...
    <div className="max-w-6xl mx-auto px-4 md:px-8 py-12 space-y-10">
            ...
            <p className="text-xl">
              ...
              {contact.email.map((mail, index) => (
                <span key={index}>
                  <a
                    href={`mailto:${mail}`}
                    className="text-[#0077C2] hover:underline transition"
                  >
                    {mail}
                  </a>
                  {index < contact.email.length - 1 && <br />}
                </span>
              ))}
            </p>
          )}
      ...
      <div className="space-y-4">
        ...
        {qualifications.map((item, index) => {
          // split at " in " to separate degree and subject
          const [degree, subject] = item.split("(");
          return (
            <p key={index} className="text-xl">
              {degree}{" "}
              {subject && (
                <span className="text-[#0077C2] font-medium"> &nbsp; {`(${subject}`}</span>
              )}
            </p>
          );
        })}
      ...
      {/* 4. Experience */}
      <div className="space-y-4">
        <h3 className="text-3xl font-semibold text-black">Experience</h3>
        <p className="text-lg text-[#686868]">{experience}</p>
      </div>
      <div className="h-[1px] w-full bg-gray-200 m-auto" />
      {/* 5. Areas of Interest */}
      <div className="space-y-4">
        <h3 className="text-3xl font-semibold text-black">Areas of Interest</h3>
