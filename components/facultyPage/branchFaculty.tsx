...
const BranchFacultyContent: React.FC = () => {
  ...
  const filteredFaculty =
    displayBranch && Array.isArray(facultyData)
      ? facultyData.filter((data: FacultyMember) => data.branch === displayBranch)
      : [];

  return (
    <div className='w-full'>
      <Title title={displayBranch || "Faculty"} className="text-7xl leading-tight font-newyork" />

      {filteredFaculty.length === 0 ? (
        <div className="text-center py-16 text-gray-500">
          <p className="text-lg">No faculty members found for {displayBranch || 'this department'}</p>
        </div>
      ) : (
